...
after = 1420
url = 'https://www.quora.com/profile/Charlie-Cheever'
has_next_page = True

def Get_questions(url, has_next_page, after):
    try:
        while has_next_page:
            s = HTMLSession()
...
def Get_question_answer(url, has_next_page, after):
    try:
        while has_next_page:
            s = HTMLSession()
            s.headers['User-Agent'] = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.114 Safari/537.36'
...
Get_question_answer(url, has_next_page, after)
